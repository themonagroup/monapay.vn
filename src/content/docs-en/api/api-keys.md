...
description: "Generate a client_secret (shown once), list and revoke keys. X-Client-Secret requirements for write calls depend on the Bearer token source."
updated: 04/09/2026
---

A MONA Pay API key is the `client_secret` string generated by `POST /api/v1/client-keys/generate`. The system shows the secret exactly once at creation and then stores only a hash, so it cannot be viewed again. Every POST, PUT, PATCH and DELETE write made with a `client/login` Bearer token must carry `X-Client-Secret`; with an `oauth/token` token the header is optional, but MONA Pay still recommends sending it. If you lose the secret, create a new key and revoke the old one; an account can have many keys.
...
| Writing data (POST, PUT, PATCH, DELETE): create VA, create QR, configure webhooks/Telegram, change password | yes | Required with a `client/login` token; recommended with an `oauth/token` token |
...
- **Token from `POST /api/v1/oauth/token`:** the server already checked `client_secret` when issuing the token, so it does not require the header again on each write. The official SDKs and MCP server still send it.
...
<figure class="photo">
  <picture>
    ...
    <img src="/img/dashboard/keys.png" width="1280" height="860" loading="lazy" decoding="async" alt="MONA Pay dashboard for creating, viewing and revoking API Keys used by systems or AI agents" />
  </picture>
  <figcaption>The dashboard lists each API key and the command block for an AI agent, captured from the my.monapay.vn dashboard.</figcaption>
</figure>
